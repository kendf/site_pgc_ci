 ...
 class Slider {
  ...
  updateSlider() {
      ...
      if (slideElements.length === 1  ) {
        ...
        // Si c'est la première slide, pas d'offset
        if (this.currentIndex === 0) {
            this.track.style.transform = `translateX(5px)`;
            
        } else {
            // Pour les autres slides, calcul normal
            const containerWidth = this.track.parentElement.offsetWidth;
            const offset = (containerWidth - slideWidth) / 2;
            const translateX = -(this.currentIndex * (slideWidth + gap)) + offset;
